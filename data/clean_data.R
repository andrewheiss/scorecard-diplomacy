...
df.orig <- read_dta(file.path(PROJHOME, "data", "original",
  ...
  mutate(tierX = ifelse(tier > 3, NA, tier),
         ...
         year = as.integer(year), cowcode = as.integer(cowcode))
...
# Load new variables
funding.raw <- read_csv(file.path(PROJHOME, "data", "original",
                                  "funding_clean.csv")) %>%
...
cables.raw <- read_dta(file.path(PROJHOME, "data", "original",
                                 "cables_panel.dta")) %>%
  mutate(cow = as.numeric(cow)) %>%
  select(cow, year, prop_tip_wl, prop_tip_estimated) %>%
  mutate(year = as.integer(year))

# Created with get_ngo_count.R
ngo.count <- read_csv(file.path(PROJHOME, "data", "original",
                                "ngo_count.csv"))
...
saveRDS(df.complete.with.lags.correct, file=file.path(PROJHOME, "data", "processed",
                                                      "df_complete.rds"))
...
reactions <- read_dta(file.path(PROJHOME, "data", "original",
                                "reactions_jk_2015-09-18.dta")) %>%
  mutate(year = as.integer(year), cowcde = as.integer(cowcode)) %>%
...
saveRDS(reactions.small, file=file.path(PROJHOME, "data", "processed",
...
# ICRG data includes three variables: government stability, internal stability,
# and external stability
df.icrg <- readRDS(file.path(PROJHOME, "data", "original", "icrg_all.rds")) %>%
