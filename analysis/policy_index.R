...
cho.orig <- read_dta(file.path(PROJHOME, "data", "original",
  ...
  mutate(year = as.integer(year),
         tier = ifelse(tier %in% c(555, 666), NA, tier),
...
year.joined <- read_csv(file.path(PROJHOME, "data", "original",
                                  "year_joined.csv"))

year.criminalized.all <- read_stata(file.path(PROJHOME, "data", "original", 
  ...
  mutate(year = as.integer(year), crimlevel = as.integer(crimlevel))
...
p.index <- cho.orig %>% left_join(year.joined, by="cow") %>%
  ...
  ungroup()
saveRDS(p.index, file.path(PROJHOME, "data", "processed", "policy_index.rds"))
...
crim.years.cases <- read_csv(file.path(PROJHOME, "data", "original", 
                                       "criminalization_cases.csv")) %>%
...
cho.crim.all.countries <- arrangeGrob(plot.years.since.full, blank,
