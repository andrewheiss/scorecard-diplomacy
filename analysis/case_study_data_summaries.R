...
write_csv(df.crim.orig, file.path(PROJHOME, "data", "processed",
                                  "crim_cases_WILL_BE_OVERWRITTEN.csv"))
df.crim <- read_csv(file.path(PROJHOME, "data", "processed",
                              "crim_cases.csv"))
...
# Export to CSV for hand refining
write_csv(leaders.cases.orig, file.path(PROJHOME, "data", "processed",
                                        "leaders_WILL_BE_OVERWRITTEN.csv"))
leaders.cases <- read_csv(file.path(PROJHOME, "data", "processed",
                                    "leaders.csv"))
...
summarize_case <- function(iso3) {
  ...
  cat(pandoc.table.return(just.numbers, justify="ll"),
      file=file.path(PROJHOME, "output", "figures", "cases",
                     paste0("summary_", iso3, ".txt")))
  
  Pandoc.convert(file.path(PROJHOME, "output", "figures", "cases",
                           paste0("summary_", iso3, ".txt")),
...
timeline_case <- function(iso3) {
  ...
  width <- 4.5
  height <- 3
  ggsave(plot.timeline, filename=file.path(PROJHOME, "output", "figures", "cases",
                                           paste0(filename, ".pdf")), 
