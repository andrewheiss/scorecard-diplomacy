...
# ------------------------
raw.years <- read.csv(file.path(PROJHOME, "data", "original",
                                "funding_original.csv"), 
                      stringsAsFactors=FALSE) %>%
  select(grant_year = fiscal.year)

funding.clean <- read.csv(file.path(PROJHOME, "data", "original",
                                    "funding_master.csv"), 
                          stringsAsFactors=FALSE) %>%
...
write_csv(funding.clean, path=file.path(PROJHOME, "data", "processed",
                                        "funding_clean.csv"))
...
igo.names <- funding.clean %>% 
  ...
  write_csv(path=file.path(PROJHOME, "data", "processed",
                           "igo_abbreviations_WILL_BE_OVERWRITTEN.csv"))

# Merge manual decicions into full dataset
funding.clean.abbrev <- funding.clean %>%
  left_join(read_csv(file.path(PROJHOME, "data", "processed",
                               "igo_abbreviations.csv")),
            by="recipient")
...
#                    "fig_grants_to_igos_collapsed_igos.txt"))
