...
# Load and clean data
# ---------------------
cables <- read_csv(file.path(PROJHOME, "data", "original",
                             "cables_trafficking.csv"))

# This file is created with PROJHOME/analysis/cables_geocode_embassies.R, 
# which takes a while to run
embassies <- readRDS(file.path(PROJHOME, "data", "processed",
                               "embassies_final.rds")) %>% select(-geocoded)
...
# TIP data (created with PROJHOME/data/clean_data.R)
...
saveRDS(cables.panel.all, file=file.path(PROJHOME, "data", "processed",
...
effort.map.binned <- ggplot(effort.full, aes(fill=bin.clean)) +
  geom_map(aes(map_id=id), map=countries.ggmap) + 
  # Second layer to add borders and slash-less legend
  geom_map(aes(map_id=id), map=countries.ggmap, size=0.15, colour="black", show.legend=FALSE) + 
  geom_point(data=embassies.to.plot, 
             aes(x=long.robinson, y=lat.robinson, fill=NULL), 
...
actual.cables.map <- ggplot(effort.full, aes(fill=num.cables)) +
  geom_map(aes(map_id=id), map=countries.ggmap) + 
  # Second layer to add borders and slash-less legend
  geom_map(aes(map_id=id), map=countries.ggmap, size=0.15, colour="black", show.legend=FALSE) + 
  geom_point(data=embassies.to.plot, 
...
estimated.cables.map <- ggplot(effort.full, aes(fill=estimated.per.year)) +
  geom_map(aes(map_id=id), map=countries.ggmap) + 
  # Second layer to add borders and slash-less legend
  geom_map(aes(map_id=id), map=countries.ggmap, size=0.15, colour="black", show.legend=FALSE) + 
  geom_point(data=embassies.to.plot, 
...
prop.present.map <- ggplot(effort.full, aes(fill=prop.present)) +
  geom_map(aes(map_id=id), map=countries.ggmap) + 
  # Second layer to add borders and slash-less legend
  geom_map(aes(map_id=id), map=countries.ggmap, size=0.15, colour="black", show.legend=FALSE) + 
  geom_point(data=embassies.to.plot, 
...
write_csv(embassies.to.csv, path=file.path(PROJHOME, "data", "processed",
                                           "map_approximate_embassy_locations.csv"))
